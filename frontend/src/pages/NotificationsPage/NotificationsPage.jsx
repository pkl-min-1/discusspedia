    ...
    return (
        <div className="notif">
            ...
            {status &&
                results.map((result) => {
                    return (
                        <div key={result.id} className={result.already_read ? "notif-info" : "notif-info info-read"} onClick={() => !result.already_read && handleClick(result.id)}>
                            {!result.already_read && <span className="badge"></span>}

                            <NotificationImportantIcon />
                            <div className="info-info">
                                <p>
                                    <span className="user">{result.name}</span> <span className="user-komen">mengomentari Postingan</span> <span>{result.post_title}</span>
                                </p>
