        ...
        <div className="profile-page">
                        ...
                        <div className="post">
                            {forumStatus &&
                                forumResult.map((result) => {
                                    return <PostUser page={"forum"} type={"post"} data={result} key={result.id} />;
                                })}
                        </div>
                        {/* Post */}
                    </TabPane>
                    <TabPane name="Survey" key="2">
                        ...
                        {surveyStatus &&
                            surveyResult.map((result) => {
                                return <PostUser page={"survey"} type={"post"} data={result} key={result.id} />;
                            })}
                    </TabPane>
                </Tabs>
            </div>
            <div className="profile">
                <div className="profile-image">
                    <img src={profileResult.avatar ? `http://localhost:8080/${profileResult.avatar}` : FotoProfile} alt="profile" />
                </div>
                <div className="profile-info">
                    {profileStatus && (
                        <>
                            ...
                            <hr />
                            <p className="role">{profileResult.role[0].toUpperCase() + profileResult.role.substring(1)}</p>
                            <hr />
                    ...
                    <div className="modal">
                        <BootstrapDialog onClose={handleClose} aria-labelledby="customized-dialog-title" open={open}>
                            ...
                            <DialogContent dividers>
                                <img src={profileResult.avatar ? `http://localhost:8080/${profileResult.avatar}` : FotoProfile} alt="Profile" width={"60px"} height={"65px"} style={{ borderRadius: "50%" }} />
                                <input type="file" name="image" accept="image/png, image/jpeg" onChange={(e) => handleChange(e.target.files[0], e.target.name)} />
