...
import useTokenStore, { useAlertStore, useConfirmStore, useEditStore } from "../../config/Store";
import Photo from "../../images/img-profile.jpg";
import "./PostContent.scss";

const PostContent = ({ data, page, type }) => {
    const [totalLike, setTotalLike] = useState(type === "comment" || page === "survey" || type === "reply" ? data.total_like : data.like_count);
    const token = useTokenStore((state) => state.token);
    const { post, del, put } = useAPI((state) => state);
    const [likeClicked, setLikeClicked] = useState(data.is_like ? data.is_like : false);
    ...
    }, [data.id, editData.comment, editData.id, editType, type, click]);
        ...
        <div className="content-container">
                ...
                {type !== "post" && (
                    <div className="user-section">
                        <div className="user-avatar">
                            <img
                                src={type === "detail" ? (data.author.profile_image ? `http://localhost:8080/${data.author.profile_image}` : Photo) : data.author_avatar ? `http://localhost:8080/${data.author_avatar}` : Photo}
                                alt="user"
                                width={"50rem"}
                                style={imgStyle}
                            />
                        ...
                        <div className="user-info">
                            <div className="user-name">{type === "comment" || type === "reply" ? data.author_name : data.author.name}</div>
                            {type !== "comment" && type !== "reply" && (
                                <div className="user-detail">
                                    <p>{data.author.role[0].toUpperCase() + data.author.role.substring(1)}</p>
                ...
                <div className="content">
                    ...
                    {type === "detail" &&
                        ...
                        data.images.map((image, index) => {
                            return <img key={index} src={`http://localhost:8080/${image.url}`} alt="Description" />;
                        })}
            ...
            <div id="bottom-section" className={type === "post" ? "post-content" : "detail-content"}>
                {type === "post" && (
                    <div className="user-section">
                        <div className="user-avatar">
                            <img src={data.author.profile_image ? `http://localhost:8080/${data.author.profile_image}` : Photo} alt="user" />
                        </div>
                        <div className="user-info">
                ...
                <div className="activity-section">
                    ...
                    <div className="like-info">
                        {type !== "post" ? likeClicked ? <FavoriteIcon onClick={clickLike} /> : <FavoriteBorderIcon onClick={clickLike} /> : <FavoriteBorderIcon onClick={clickLike} />}
                        {totalLike}
                    </div>
                    <div className="comment-info">
                        ...
                        {type === "comment" || type === "reply" ? data.total_reply : page === "survey" ? data.total_comment : data.comment_count}
                    </div>
                </div>
            </div>
            {commentClicked && (
                <form className="input-section">
                    ...
                    <div className="flex-9">
                        <textarea name="comment" className="comment-input" rows="5" onChange={(e) => handleChange(e.target.value, e.target.name)} value={userData.comment ? userData.comment : ""}></textarea>
